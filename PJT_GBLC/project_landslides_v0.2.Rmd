...
# Contar ocorrências de duplicatas
total_duplicatas <- df_gblc %>%
  group_by_all() %>%
  summarize(count = n()) %>%
  filter(count > 1)
...
```{r}
# Selecionando e excluindo colunas irrelevantes
df_gblc <- df_gblc %>% select(-source_name, 
         -source_link, 
         -event_title,
         -event_description, 
         -storm_name,
...
#contagem de eventos por ano
df_gblc$event_year <- year(mdy_hms(df_gblc$event_date))
eventos_por_ano <- table(df_gblc$event_year)
barplot(eventos_por_ano, main="Eventos por Ano", xlab="Ano", ylab="Número de Eventos")
```

```{r}
ggplot(df_gblc, aes(x=factor(event_year), y=..count..)) +
  geom_bar(stat="count") +
  labs(title="Eventos por Ano", x="Ano", y="Número de Eventos") +
  scale_y_continuous(breaks=seq(0, max(df_gblc$event_year), by=500)) +
  theme_minimal()
```

```{r}
ggplot(df_gblc, aes(x = factor(event_year), group = 1)) +
  geom_line(stat = "count", color='blue') +  
  labs(title = "Eventos por Ano", x = "Ano", y = "Número de Eventos") +
  scale_y_continuous(breaks = seq(0, max(df_gblc$event_year), by = 100)) +
  theme_minimal()
```
...
ggplot(df_gblc, aes(x = factor(event_year), group = 1)) +
  ...
  geom_point(stat = "count", color = '#ce2340', size = 2) +
  labs(title = "Eventos por Ano", x = "Ano", y = "Número de Eventos") +
  scale_y_continuous(breaks = seq(0, max(df_gblc$event_year), by = 100)) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 20, hjust = 0.5, face = "bold", color = "navy"),
